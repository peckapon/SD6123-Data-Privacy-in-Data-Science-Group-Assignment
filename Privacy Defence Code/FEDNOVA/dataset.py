...
Provides train/test loaders and partitioning for federated learning.
"""
...
def load_datasets(config: DictConfig) -> Tuple[List[DataLoader], DataLoader, List]:
    ...
    # Keys are client ids (as strings), values are lists of CIFAR-10 train indices.
    import json, os
    partition_path = os.path.join(
        config.datapath,
        f"partition_indices_seed{config.seed}_alpha{config.alpha}"
