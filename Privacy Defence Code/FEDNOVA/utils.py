...
def comp_accuracy(output, target, topk=(1,)):
    """
    Compute accuracy over the k top predictions with respect to the target.
    """
    with torch.no_grad():
        maxk = max(topk)
...
def fit_config(exp_config: DictConfig, server_round: int):
    ...
    Learning rate follows a two-step decay:
    - at 50% of total rounds: lr = base_lr / 2
    - at 80% of total rounds: lr = base_lr / 4
    """
    config = {}
    ...
    if exp_config.lr_scheduling:
        if server_round >= int(exp_config.num_rounds * 0.8):
            lr = exp_config.optimizer.lr / 4

        elif server_round >= int(exp_config.num_rounds / 2):
            lr = exp_config.optimizer.lr / 2
