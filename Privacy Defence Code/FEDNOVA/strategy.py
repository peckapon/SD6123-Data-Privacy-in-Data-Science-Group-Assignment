...
class FedAvgWithCommTracking(FedAvg):
    ...
    def aggregate_fit(
        ...
        dropped_soft_failures = sum(
            1
            for _, fit_res in results
            if int(getattr(fit_res, "num_examples", 0)) <= 0
            or int(fit_res.metrics.get("dp_failed_oom", 0)) > 0
            or int(fit_res.metrics.get("failed_oom", 0)) > 0
        )
        hard_failures = len(failures)
        failed_total = dropped_soft_failures + hard_failures
        record["clients_selected"] = len(results) + hard_failures
        record["clients_returned"] = len(results)
        record["clients_dropped_soft"] = dropped_soft_failures
        record["clients_failed_hard"] = hard_failures
        record["clients_failed_total"] = failed_total
        record["clients_effective"] = len(results) - dropped_soft_failures
        log(
            INFO,
            "[Round %d] Client outcomes — returned: %d | effective: %d | "
            "failed_total: %d (dropped_soft: %d, failed_hard: %d)",
            server_round,
            len(results),
            len(results) - dropped_soft_failures,
            failed_total,
            dropped_soft_failures,
            hard_failures,
        )
        ...
        if epsilon_vals:
            sample_fit = next(
                (
                    fit_res
                    for _, fit_res in results
                    if int(fit_res.metrics.get("dp_num_steps", 0)) > 0
                ),
                results[0][1],
            )
            self.epsilon_log.append({
                "round": server_round,
                "mean_epsilon_this_round": float(np.mean(epsilon_vals)),
                "dp_noise_multiplier": float(sample_fit.metrics.get("dp_noise_multiplier", 0.0)),
                "dp_sample_rate": float(sample_fit.metrics.get("dp_sample_rate", 0.0)),
                "dp_num_steps": int(sample_fit.metrics.get("dp_num_steps", 0)),
            })
